...
controllers by an 'x-api-version' HTTP header. When a route uses Constraints, each constraint has a chance
to prevent the route from matching a given request. In this sample, a custom RouteProviderAttribute 
...
The custom constraint implementation (VersionConstraint) is implemented based on the value of 'x-api-version'
matching an integer value. The value of the allowed version for the constraint is provided by the VersionedRoute
attribute placed on each controller. When a request comes in, the header value of 'x-api-version' is matched
